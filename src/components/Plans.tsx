  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Meal Plans</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4">
        {days.map((day) => (
          <div key={day} className="space-y-3 sm:space-y-4">
            <h3 className="text-left sm:text-center font-bold text-gray-700 text-xs sm:text-sm uppercase tracking-wider border-b border-gray-100 sm:border-0 pb-1 sm:pb-0">{day}</h3>
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              {meals.map((meal) => (
                <div key={meal} className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm min-h-[80px] sm:min-h-[100px] hover:border-orange-200 transition-colors cursor-pointer">
